    ...
    ): Promise<NAISDK.AgenticMoveResult> =>
    {
        ...
        const tools = {
            get_legal_moves: tool({
                description: "Get the current position (FEN), turn, and all legal moves in SAN.",
            ...
            make_move: tool({
                description: "Submit a move in SAN. Returns success or error.",
                inputSchema: z.object({
                    move: z.string().describe("Move in SAN (e.g. 'Nf3', 'e4', 'O-O')"),
                    offerDraw: z.boolean().describe("Offer a draw"),
                    resign: z.boolean().describe("Resign"),
                }),
                execute: async ({ move, offerDraw, resign }): Promise<MakeMoveResult> => {
                    ...
                    return {
                        success: false,
                        error: `"${move}" is not legal. Call get_legal_moves to see valid moves.`,
                    };
                },
            }),
        };
        ...
        const { data: result, error } = await tryCatch(generateText({
            model: openrouter.chat(model.version),
            system: systemPrompt,
            prompt: `FEN: ${fen}\nTurn: ${turn}\n\nCall get_legal_moves, then make_move.`,
            tools,
